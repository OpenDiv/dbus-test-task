...
int main(int argc, char *argv[]) {
  ...
  if (!systemTimeReply.isValid()) {
    ...
    if (systemTimeReply.value() > QDBusLastErrorCode) {
      QDateTime dateTime =
          QDateTime::fromMSecsSinceEpoch(systemTimeReply.value());
      qDebug() << dateTime.toString("yyyy-MM-dd HH:mm:ss");
      return 0;
    } else if (systemTimeReply.value() ==
               static_cast<quint64>(QDBusError::AccessDenied)) {
      ...
      qDebug() << "Something is wrong with com.system.time, check secondTask "
